...
Paparazzo.prototype.setup = function (callback) {
  ...
  semlocks.acquire("phantom", { wait: 30000 }, function (err, release) {
    ...
    phantom.create("--ignore-ssl-errors=yes", function (pageHelper) {
...
Paparazzo.prototype.capture = function (target, options, callback) {
  ...
  var myTimeoutId = setTimeout(function () {
    isTimedOut = true;
    var err = new Error('Phantom timed out after 30 seconds of trying to open and capture a single page: ' + self.targetUrl);
    callback(err);
  }, 30000) // 30 seconds
...
Paparazzo.prototype.navigateToPage = function (callback) {
  ...
  var operation = retry.operation({retries: 3,
                                   maxTimeout: 5000});
  operation.attempt(function (current_attempt) {
    self.page && self.page.open(self.targetUrl, function (status) {
      var err = undefined;
      if (status !== "success") {
        err = "\n" + "Failed to open '" + status + "' [" + self.targetUrl + "]"
      }
      if (!isTimedOut && operation.retry(err)) {
        return;
      }
